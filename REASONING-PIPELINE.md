...
> Note on the shipped version. This document is the analysis of the target, not the log of the final code. At build time, two choices were settled against the recommendations above, with verification in the official docs to back them: the self-critique hook (Stop) keeps `prompt` rather than the `agent` recommended in section 4.7 (the docs mark `agent` hooks as experimental), and the intention hook (PreToolUse) is a `command` filtered to grave gestures rather than a `prompt`/`agent`. Details in `docs/DOCTRINE.md` and `README.md`.
